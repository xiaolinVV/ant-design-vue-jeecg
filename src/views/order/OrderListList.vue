<template>
  <a-card :bordered="false">
    ...
    <!-- 操作按钮区域 -->
    <div class="table-operator">
      <a-button type="primary" icon="download"  :loading="exportLoading"  @click="handleExportXls('订单-平台订单-全部订单')">导出</a-button>
    </div>
    ...
    <div>
      ...
      >
        ...
        <template slot="customaryDues" slot-scope="text, record, index">
          <div>
            {{ record.customaryDues < 0 ? 0 : record.customaryDues }}
          </div>
        </template>
        ...
        <template slot="coupon" slot-scope="text, record, index">
          <div>
            <a v-if="record.coupon" @click="showModalCouponInformation(record)">{{ record.coupon }}</a>
            <a v-else>-</a>
          </div>
        </template>
