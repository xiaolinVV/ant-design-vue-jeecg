<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button type="primary" icon="download"  :loading="exportLoading"  @click="handleExportXls('订单-店铺订单-待收货订单')">导出</a-button>
    </div>
    <!--<div class="table-operator">
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/orderStoreList/orderStoreList/queryPageListWaitForReceiving',
        delete: '/orderStoreList/orderStoreList/delete',
        deleteBatch: '/orderStoreList/orderStoreList/deleteBatch',
        exportXlsUrl: 'orderStoreList/orderStoreList/exportPageListWaitForReceiving',
  ...
  methods: {
    ...
    particulars(record, reusePage) {
      ...
      this.$router.push({ path: '/order/OrderStoreDetails', query: { record: record, reusePage: reusePage } }) //reusePage=4
    },
    //发货
    deliverGoods(record) {
      localStorage.setItem('deliverGoods', JSON.stringify(record))
      this.$router.push({ path: '/order/modules/StoreOrderDeliverGoods', query: { record: record } }) //reusePage=4
    },
    //查看物流
