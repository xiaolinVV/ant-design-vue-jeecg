<template>
  <a-card :bordered="false">
    ...
    <!--<a-button type="primary" icon="download" @click="orderListExport(1)">导出预览</a-button>-->
    <!--</div>-->

    <!-- 操作按钮区域 -->
    <div class="table-operator">
      <a-button type="primary" icon="download" @click="handleExportXls('订单-平台订单-待发货订单')">导出</a-button>
    </div>
...
export default {
  ...
  methods: {
    ...
    particulars(record, reusePage) {
      ...
      this.$router.push({ path: '/order/OrderDetails', query: { record: record, reusePage: reusePage } }) //reusePage=3
    },
    //发货
    deliverGoods(record) {
