<template>
  <a-card :bordered="false">
    ...
    <!--<a-button type="primary" icon="download"  :loading="exportLoading"  @click="orderListExport(2)">导出预览</a-button>-->
    <!--</div>-->

    <!-- 操作按钮区域 -->
    <div class="table-operator">
      <a-button type="primary" icon="download"  :loading="exportLoading"  @click="handleExportXls('订单-平台订单-待收货订单')">导出</a-button>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/orderList/orderList/queryPageListWaitForReceiving',
        delete: '/orderList/orderList/delete',
        deleteBatch: '/orderList/orderList/deleteBatch',
        exportXlsUrl: 'orderList/orderList/exportPageListWaitForReceiving',
  ...
  methods: {
    ...
    particulars(record, reusePage) {
      ...
      this.$router.push({ path: '/order/OrderDetails', query: { record: record, reusePage: reusePage } }) //reusePage=4
    },
    //发货
    deliverGoods(record) {
      localStorage.setItem('deliverGoods', JSON.stringify(record))
      this.$router.push({ path: '/order/modules/OrderDeliverGoods', query: { record: record } }) //reusePage=4
    },
    //查看物流
