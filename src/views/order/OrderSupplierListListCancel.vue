<template>
  <a-card :bordered="false">
    ...
    <div>
      ...
      "
            >{{ record.orderList.memberPhone }}</a
            >
          </div>
        </template>
        ...
        <div>
        <a v-if="record.orderList.message" @click="showModalMessageInformation(record.orderList.message)">留言</a>
        <a v-else>-</a>
        </div>
        </template>
        ...
        <template slot="isUpdateAddr" slot-scope="text, record, index">
          <div>
            {{record.orderList.isUpdateAddr==0?'是':'否'}}
          </div>
        </template>
        ...
        <template slot="action" slot-scope="text, record">
          ...
          <div v-if="record.status == '0'">
            ...
            <a
              @click="showModalAddressInformation(record.orderList.id,record.orderList.memberListId,record.orderList.consignee,record.orderList.contactNumber,record.orderList.shippingAddress)">修改地址</a>
            <a-divider type="vertical"/>
          ...
          <div v-if="record.status == '3'">
            ...
            <a-divider type="vertical" v-if="record.orderEvaluateDTOList.length > 0"/><!--v-if="record.isEvaluate == 1"-->
  ...
  export default {
    ...
    data() {
      return {
        ...
        queryParam: {//初始查询条件
          status: 4 //订单状态；0：待付款；1：待发货（已付款、部分发货）；2：待收货（已发货）；3：交易成功；4：交易失败；5：交易完成；
        },
        // 表头
        columns: [
          {
    ...
    methods: {
      ...
      showModalEmemberInformation(memberList, consignee, contactNumber, shippingAddress, houseNumber) {
        this.$refs.modalForm.showModalEmemberInformation(memberList, consignee, contactNumber, shippingAddress, houseNumber)
      ...
      orderListExport(status) {
        ...
        let HOST = window.location.protocol + '//' + window.location.host + '/jeecg-boot/ureport/preview?'
