<template>
  <a-card :bordered="false">
      ...
      <a-spin :spinning="confirmLoading">
        <a-form :form="form">
          ...
          <a-form-item
            ...
            label="订单类型；0：普通订单；1：拼团订单；2：抢购订单；3：自选订单；4：兑换订单；数据字典：order_type；"
          >
            <a-input
              placeholder="请输入订单类型；0：普通订单；1：拼团订单；2：抢购订单；3：自选订单；4：兑换订单；数据字典：order_type；"
          ...
          <a-form-item
            :labelCol="labelCol"
            :wrapperCol="wrapperCol"
            label="关闭类型；0：超时关闭；1：买家关闭；2：卖家关闭；数据字典：oder_close_type；"
          >
            <a-input
              placeholder="请输入关闭类型；0：超时关闭；1：买家关闭；2：卖家关闭；数据字典：oder_close_type；"
              v-decorator="['closeType', {}]"
            />
          </a-form-item>
          <a-form-item
            :labelCol="labelCol"
            :wrapperCol="wrapperCol"
            label="订单关闭原因；超时关闭：原因0：超时未付款；买家关闭：原因1：订单不能按预计时间送达；原因2：操作有误（商品、地址等选错）；原因3：重复下单/误下单；原因4：其他渠道价格更低；原因5：不想买；原因6：商品无货；原因7：其他原因；卖家关闭：原因8：未及时付款；原因9：买家不想买了 ；原因10：买家信息填写错误，重新拍；原因11：恶意买家/同行捣乱；原因12：缺货；原因13：同城见面交易；原因14：其他原因；数据字典：oder_close_explain"
          >
            <a-input
              placeholder="请输入订单关闭原因；超时关闭：原因0：超时未付款；买家关闭：原因1：订单不能按预计时间送达；原因2：操作有误（商品、地址等选错）；原因3：重复下单/误下单；原因4：其他渠道价格更低；原因5：不想买；原因6：商品无货；原因7：其他原因；卖家关闭：原因8：未及时付款；原因9：买家不想买了 ；原因10：买家信息填写错误，重新拍；原因11：恶意买家/同行捣乱；原因12：缺货；原因13：同城见面交易；原因14：其他原因；数据字典：oder_close_explain"
              v-decorator="['closeExplain', {}]"
            />
          </a-form-item>
          ...
          <a-form-item
            :labelCol="labelCol"
            :wrapperCol="wrapperCol"
            label="订单配送方式；0：快递；1：自提；2：配送；数据字典：oder_distribution"
          >
            <a-input
              placeholder="请输入订单配送方式；0：快递；1：自提；2：配送；数据字典：oder_distribution"
              v-decorator="['distribution', {}]"
            />
          </a-form-item>
          ...
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="商品利润">
    ...
    <a-modal
      ...
      cancelText="取消"
    >
      <div v-for="item of formList" style="margin-bottom: 20px">
        <div class="suppliers">
          <div>
            <span>供应商</span>
        ...
        <a-table
          ...
          :columns="columns"
          :pagination="false"
          :dataSource="item.orderStoreGoodRecordDTOList"
        >
    ...
    <!--评论弹出-->
    <a-modal title="查看评论" :footer="null" :width="700" v-model="visiblEvaluateInformation">
      <div>
        <!--评论内容-->
        <div>
          <div style="margin-bottom: 20px" v-for="item in orderListDTO.orderEvaluateStoreDTOList">
            <div style="display: flex;justify-content:flex-start;">
              ...
              <div class="neirongCSS">
                ...
                <div>
                  描述相符：
                  <template>
                    <a-rate class="colorSize" :defaultValue="item.descriptionStar" disabled />
                  </template>
                </div>
              </div>
            </div>
    ...
    >
      <div style="margin-bottom: 20px" v-if="marketingDiscountCoupon">
        ...
        <div class="CouponTextA">
          ...
          <div class="CouponText2" lenght="50">
      ...
      <!--&lt;!&ndash;            {{ marketingDiscountCoupon.name }}&ndash;&gt;-->
      <!--</div>-->
      <!--<div class="CouponText">优惠金额</div>-->
      <!--<div class="CouponText2">-->
      <!--&lt;!&ndash;            {{ marketingDiscountCoupon.regulation }}&ndash;&gt;-->
      <!--</div>-->
      <!--</div>-->
      <!--</div>-->
    </a-modal>
...
export default {
  ...
  data() {
    return {
      ...
      visiblPreferentialInformation: true, //优惠信息
      orderId: '',
      listMemberShippingAddress: [],
      orderProviderListDTOs: [], //配送信息
      storeTemplateMaps: [], //运费模板信息
      areaAddressId: '', //修改地址
      ...
      validatorRules: {},
      url: {
        add: '/orderStoreList/orderStoreList/add',
        edit: '/orderStoreList/orderStoreList/edit',
        imgerver: window._CONFIG['domianURL'] + '/sys/common/view',
        memberListIdList: '/memberShippingAddress/memberShippingAddress/memberListIdList',
        updateAddress: '/orderStoreList/orderStoreList/updateAddress',
        reviewInformation: '/orderEvaluateStore/orderEvaluateStore/reviewInformation',
        updateAuditStatus: '/orderEvaluateStore/orderEvaluateStore/updateStatus'
      }
    }
  },
  created() {},
  ...
  methods: {
    ...
    showModalCouponInformation(marketingDiscountCoupon, welfarePayments, vipLowerTotal, memberList) {
      this.marketingDiscountCoupon = marketingDiscountCoupon
      this.welfarePayments = welfarePayments
      this.vipLowerTotal = vipLowerTotal
      if (memberList) {
        //判断会员信息
        this.memberType2 = memberList.memberType
      }
    ...
    showModalDistributionInformation(storeTemplateMaps) {
      ...
      this.storeTemplateMaps = storeTemplateMaps
    },
