...
-->
    ...
    >
      <div>
        <!--评论内容-->
        <div>
          <div style="margin-bottom: 20px" v-for="item in orderProviderListDTO.orderEvaluateDTOList">
        ...
        <div style="background-color: #f2f2f2;padding:20px;">
          ...
          <div>
            <div style="margin-bottom: 10px">
              物流服务：
              <template>
                <a-rate class="colorSize" v-model="orderProviderListDTO.orderList.logisticsStar" disabled />
              </template>
            </div>
            <div style="margin-bottom: 10px">
              发货速度：
              <template>
                <a-rate class="colorSize" v-model="orderProviderListDTO.orderList.shippingStar" disabled />
              </template>
            </div>
            <div style="margin-bottom: 10px">
              服务态度：
              <template>
                <a-rate class="colorSize" v-model="orderProviderListDTO.orderList.serviceStar" disabled />
              </template>
            </div>
          </div>
        </div>
        <!--评论人-->
        <div class="pinlunren">
          <div>评论人：{{ orderProviderListDTO.orderList.nickName }}</div>
          <div>评论时间：{{ orderProviderListDTO.orderList.evaluateTime }}</div>
        </div>
      </div>
    </a-modal>
    ...
    >
      <div style="margin-bottom: 20px" v-if="marketingDiscountCoupon">
        ...
        <div class="CouponTextA">
          ...
          <div class="CouponText2" v-if="marketingDiscountCoupon.publisher == 0">平台</div>
          <div class="CouponText2" v-if="marketingDiscountCoupon.publisher == 1">店铺</div>
          <div class="CouponText2" v-else>--</div>
          <div class="CouponText">优惠券有效期</div>
      ...
      <div style="margin-bottom: 20px;">
        ...
        <div class="CouponTextA">
          <div class="CouponText">会员等级</div>
          <div class="CouponText2">
            <!--            {{ marketingDiscountCoupon.name }}-->
          </div>
          <div class="CouponText">优惠金额</div>
          <div class="CouponText2">
            <!--            {{ marketingDiscountCoupon.regulation }}-->
          </div>
        </div>
      </div>
    </a-modal>
...
export default {
  ...
  methods: {
    ...
    showEvaluateModal(record) {
      // var that = this
      // getAction(that.url.reviewInformation, {
      //   orderListId: orderListId,
      //   orderProviderListId: orderProviderListId,
      //   type: '1'
      // }).then((res) => {
      //   if (res.success) {
      ...
      this.orderProviderListDTO = record
