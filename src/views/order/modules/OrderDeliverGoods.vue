...
<!--                            :src="getAvatarView(record.mainPicture)" icon="user"/>-->
              ...
              <template slot="parcel" slot-scope="text, record, index">
                <div class="anty-img-wrap">
                  <a-select style="width:100px" v-model="orderProviderListDTOs[menuCurrent[0]].OrderProviderGoodRecordId[index]" placeholder="请选择">
                    <a-select-option :value="0" @click="ChooseParcel(record,'请选择')">请选择</a-select-option>
                    <a-select-option v-for="(selectItem,selectIndex) in orderProviderListDTOs[menuCurrent[0]].listParcel" :value="selectIndex + 1" @click="ChooseParcel(record,selectItem)">
                      {{selectItem.name}}
                    </a-select-option>
      ...
      <a-card class="wrap">
        ...
        <a-card :bordered="true" class="head-decoration-parcel">
          <div>
            <span>我的发货信息:</span>
            <span class="head-decorationGoods">{{ orderProviderListDTOs[menuCurrent[0]].providerAddressFa }}</span><!--闫小小 ，18750202107 ，0592-5924551，福建省厦门市思明区软件园二期望海路10号之二xx单元-->
            <span style="color: #2eabff;margin-left: 40px" @click="showModalAddressInformationFaTui(orderProviderListDTOs[menuCurrent[0]].listParcel[0].id,orderProviderListDTOs[menuCurrent[0]].providerAddressFa,'1')">修改</span>
          </div>
          <div>
            <span>我的退货信息:</span>
            <span class="head-decorationGoods">{{ orderProviderListDTOs[menuCurrent[0]].providerAddressTui }}</span><!--闫小小 ，18750202107 ，0592-5924551，福建省厦门市思明区软件园二期望海路10号之二xx单元-->
            <span style="color: #2eabff;margin-left: 40px" @click="showModalAddressInformationFaTui(orderProviderListDTOs[menuCurrent[0]].listParcel[0].id,orderProviderListDTOs[menuCurrent[0]].providerAddressTui,'2')" >修改</span>
          </div>
        </a-card>
...
//**********************************数据渲染开始********************************************//
    ...
    methods: {
     ...
     //发货提交
      ...
      //取消按钮
      cancel(){
        this.$router.push({ path: '/order/OrderListListToSendTheGoods' })//reusePage=4
