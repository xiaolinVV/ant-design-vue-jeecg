  ...
  <div class="order-details">
    ...
    <a-card class="wrap">
      <div style="display: flex;justify-content:space-between;">
        ...
        >
          <a-menu-item style="width: 150px;text-align: center;"
                       v-for="(itemOp,indexOp) in orderStoreSubListDTOs" :key="indexOp"
                       v-if="orderStoreSubListDTOs[indexOp].goodsInfo && orderStoreSubListDTOs[indexOp].goodsInfo.length > 0"
          >{{ itemOp.sysUserName }}
          </a-menu-item>
        </a-menu>
        <div class="order-number">
          ...
          <div>
            {{ orderStoreSubListDTOs[menuCurrent[0]].orderNo }}
          </div>
        </div>
      ...
      <div>
        ...
        <a-card :bordered="true" class="head-decoration-parcel selectService"
                v-for=" (itemParcel,indexParcel) in orderStoreSubListDTOs[menuCurrent[0]].listParcel" >
          <div style="display:flex;justify-content: space-between">
          ...
          <div>
            <span>包裹商品:</span>
            <a-tag   v-for="(storeItem,storeIndex) in itemParcel.orderProviderGoodRecordInfo" style="margin: 15px 0 0 15px;">
              {{ storeItem.name }}
        ...
        <div class="fahuoGoods" style="border: 1px solid #e6e6e6;margin-top: 20px;padding: 20px">
          ...
          <a-table
            ...
            :dataSource="orderStoreSubListDTOs[menuCurrent[0]].goodsInfo"
            :pagination="false"
            @change="handleTableChange">
            <template slot="mainPicture" slot-scope="text, record, index">
              <div class="anty-img-wrap">
                <a-avatar shape="square" v-if="record.mainPicture != undefined &&  record.mainPicture == ''" icon="user"/>
            ...
            <template slot="parcel" slot-scope="text, record, index">
              <div class="anty-img-wrap">
                <a-select style="width:100px" v-model="orderStoreSubListDTOs[menuCurrent[0]].OrderProviderGoodRecordId[index]" placeholder="请选择">
                  <a-select-option :value="0" @click="ChooseParcel(record,'请选择')">请选择</a-select-option>
                  <a-select-option v-for="(selectItem,selectIndex) in orderStoreSubListDTOs[menuCurrent[0]].listParcel" :value="selectIndex + 1" @click="ChooseParcel(record,selectItem)">
                    {{selectItem.name}}
                  </a-select-option>
    ...
    <a-card class="wrap">
      ...
      <a-card :bordered="true" class="head-decoration-parcel">
        <div>
          <span>我的发货信息:</span>
          <span class="head-decorationGoods">{{ orderStoreSubListDTOs[menuCurrent[0]].providerAddressFa }}</span><!--闫小小 ，18750202107 ，0592-5924551，福建省厦门市思明区软件园二期望海路10号之二xx单元-->
          <span style="color: #2eabff;margin-left: 40px" @click="showModalAddressInformationFaTui(orderStoreSubListDTOs[menuCurrent[0]].listParcel[0].id,orderStoreSubListDTOs[menuCurrent[0]].providerAddressFa,'1')">修改</span>
        </div>
        <div>
          <span>我的退货信息:</span>
          <span class="head-decorationGoods">{{ orderStoreSubListDTOs[menuCurrent[0]].providerAddressTui }}</span><!--闫小小 ，18750202107 ，0592-5924551，福建省厦门市思明区软件园二期望海路10号之二xx单元-->
          <span style="color: #2eabff;margin-left: 40px" @click="showModalAddressInformationFaTui(orderStoreSubListDTOs[menuCurrent[0]].listParcel[0].id,orderStoreSubListDTOs[menuCurrent[0]].providerAddressTui,'2')" >修改</span>
        </div>
      </a-card>
    ...
    <!--修改供应商发货退货地址弹出-->
    <a-modal title="修改买家收货地址" :width="700" v-model="AddressFaTui" @ok="hideModalAddressFaTuiInformation()" okText="确认" cancelText="取消">
      <div>
        <div class="Address" >
          当前订单的收货信息：{{ Addressft }}
        </div>
        ...
        <div style="margin-top: 60px;margin-bottom: 60px">
          <a-radio-group @change="onChange" v-model="providerAddressId">
            <a-radio :style="radioStyle"  v-for="item of listProviderAddressDTO"  :value="item.id">
              {{ item.contactName }}&nbsp;{{ item.contactPhone }}&nbsp&nbsp;{{ item.contactTelephone }}&nbsp;{{ item.detailedAddress }}
            </a-radio>
          </a-radio-group>
    ...
    data() {
      return {
        ...
        url: {
          ...
          ordereDlivery:"/orderStoreList/orderStoreList/ordereDlivery",
          ProviderAddressDTOList:"/storeAddress/storeAddress/StoreAddressDTOList",
          updateProviderAddressId:"/orderStoreSubList/orderStoreSubList/updateProviderAddressId"
        },
      }
    },
    computed: {
...
//**********************************数据渲染开始********************************************//
        ...
        //商品信息
        let orderStoreSubListDTOs = data.orderStoreSubListDTOs
        let sz = []
        for (let itemOPL of orderStoreSubListDTOs) {
          let gongyingshang = {
            //供应商姓名
          ...
          gongyingshang.goodsInfo = itemOPL.orderStoreGoodRecords
        ...
        this.orderStoreSubListDTOs = sz
        // console.log("orderStoreSubListDTOs---", orderStoreSubListDTOs);
        /*********************************包裹添加未处理**************************************************/
        ...
        for(let index = 0;index<this.orderStoreSubListDTOs.length;index++){
          if(this.orderStoreSubListDTOs[index].goodsInfo.length > 0 ){
    ...
    methods: {
      ...
      ChooseParcel(record,item) {
        let listParcel = this.orderStoreSubListDTOs[this.menuCurrent[0]].listParcel
        let info = {
      ...
      addParcel() {
        ...
        this.orderStoreSubListDTOs[this.menuCurrent[0]].listParcel.push(packages)
      },
      //删除包裹提示框
      showConfirm(index) {
        let that = this
      ...
      deletePackage(index) {
        count--
        this.orderStoreSubListDTOs[this.menuCurrent[0]].listParcel.splice(index, 1)
      },
      //删除标签事件
      deleteTag(item){
        let id = item.id
        this.orderStoreSubListDTOs[this.menuCurrent[0]].goodsInfo.forEach((item,index)=>{
          if(id == this.orderStoreSubListDTOs[this.menuCurrent[0]].goodsInfo[index].id){
            this.ChooseParcel(item,'请选择')
            this.$set(this.orderStoreSubListDTOs[this.menuCurrent[0]].OrderProviderGoodRecordId,index,0);
            return
          }
        })
      },
      ...
      deliverGoodsSubmit(){
        let that = this
        let hasPackage = 0
        for(let data of this.orderStoreSubListDTOs){
        ...
        setTimeout(()=>{
          ...
          that.$confirm({
            ...
            content: '您确定要提交吗？',
            onOk() {
              postAction(that.url.ordereDlivery, {listMap:that.orderStoreSubListDTOs}).then((res) => {
                if (res.success) {
                  that.$message.success(res.message);
                  that.$router.push({ path: '/order/OrderStoreListListToSendTheGoods'})
      ...
      hideModalAddressFaTuiInformation(){
        ...
        getAction(that.url.updateProviderAddressId, {id:that.orderProviderListId,providerAddressId:that.providerAddressId,AddressftIndex:that.AddressftIndex}).then((res) => {
          if (res.code==200) {
            that.$message.success(res.message);
      ...
      //取消按钮
      cancel(){
        this.$router.push({ path: '/order/OrderStoreListListToSendTheGoods' })//reusePage=4
