...
export default {
  ...
  methods: {
    ...
    particulars(record, reusePage) {
      ...
      this.$router.push({ path: '/order/OrderDetails', query: { record: record, reusePage: reusePage } }) //reusePage=5
    },
    //发货
    deliverGoods(record) {
      localStorage.setItem('deliverGoods', JSON.stringify(record))
      this.$router.push({ path: '/order/modules/OrderDeliverGoods', query: { record: record } }) //reusePage=5
    },
    //查看物流
    checkLogistics(record, reusePage) {
      localStorage.setItem('deliverGoods', JSON.stringify(record))
      this.$router.push({ path: '/order/OrderLogisticsGoods', query: { record: record, reusePage: reusePage } }) //reusePage=5
    },
