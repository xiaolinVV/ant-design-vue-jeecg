      ...
      <div>
        <a-menu
         ...
         fontSize:'18PX',
          ...
          <a-menu-item :key="index" v-for="(orderProviderListDTO,index) in orderProviderListDTOs"
                       @click="theParcelInformation(orderProviderListDTO,index)">包裹 {{ index+1 }}
          </a-menu-item>
        ...
        <div>
          ...
          <div style="margin-top: 30px">
            <template v-if="logisticsTracking">
              <a-timeline>
                <a-timeline-item color="red" v-for="item in logisticsTrackingList">{{ item.time }} {{ item.status }}
                </a-timeline-item>
              </a-timeline>
            </template>
            <template v-else>
      ...
      <div>
        ...
        <div class="fahuoGoods" style="border: 1px solid #e6e6e6;margin-top: 20px;padding: 20px">
          ...
          <a-table
            ...
            rowKey="id"
            :columns="columns"
            :dataSource="orderProviderGoodRecords"
            :pagination="false"
            :loading="loading"
    ...
    data() {
      return {
        ...
        url: {
          ...
          parcelInformation: '/orderProviderList/orderProviderList/parcelInformation'
        }
      }
    },
    computed: {
...
//**********************************数据渲染开始********************************************//
        ...
        /* let  orderProviderListDTOs = this.orderProviderListDTOs
    ...
    methods: {
      ...
      parcelInformation(id) {
        console.log('id++++', id)
        var that = this
        getAction(that.url.parcelInformation, { orderListId: id, type: '1' }).then((res) => {
          if (res.success) {
            this.orderProviderListDTOs = res.result
            if (this.orderProviderListDTOs.length > 0) {
              this.orderProviderList = this.orderProviderListDTOs[0]
              //包裹订单点击渲染信息
      ...
      theParcelInformation(orderProviderList, index) {
        ...
        if (orderProviderList) {
          ...
          this.card1Cnt = [
            ...
            this.bgIndex = index
          //当前商品信息
          this.orderProviderGoodRecords = orderProviderList.orderProviderGoodRecords
          //快递公司
