    ...
    <div class="order-details">
      ...
      <a-card :bordered="true" class="head-decoration">
        ...
        <div v-if="reusePage == 5">
          ...
          <div style="float: right">
            <a-button type="primary" v-if="record.isEvaluate == 1" @click="showAudiModal(record.id)"
                      style="margin-right: 20px">审批
            </a-button><!--v-if="record.isEvaluate == 1"-->
            <a-button type="primary" v-if="record.isEvaluate == 1" @click="showEvaluateModal(record.id)"
                      style="margin-right: 20px">查看评价
            </a-button><!--v-if="record.isEvaluate == 1"-->
      ...
      <a-card class="wrap">
        <detail-list :col="4" style="margin-bottom: 36px;">
          <detail-list-item v-for='item in card3Cnt' :term="item.term">
            {{item.content}}
          </detail-list-item>
        </detail-list>
        <detail-list :col="4" style="margin-bottom: 36px;">
      ...
      <!--<a-card class="wrap">
        <detail-list  :col="4" style="margin-bottom: 36px;">
          <detail-list-item v-for='item in card7Cnt' :term="item.term" >
            {{item.content}}
          </detail-list-item>
        </detail-list>
      </a-card>-->
...
<script>
  ...
  for (let a = 0;a<15;a++){
    data1.push({
      commodityPictures: 'https://timgsa.baidu.com/timg?image&quality=80&size=b9999_10000&sec=1574006779479&di=0a077b6237235d483438a60d409bcb88&imgtype=jpg&src=http%3A%2F%2Fimg4.imgtn.bdimg.com%2Fit%2Fu%3D1535247749%2C658680188%26fm%3D214%26gp%3D0.jpg',
      name: `矿泉水 ${ 550 + a}ml`,
      Specifications: `12421432143214321${a}`,
      price: `${a + 1}.00`,
      num: a,
      amount: `${a + 3}`,
      weight:`${a * 10 }g`
    ...
    data2.push({
      distributionMode: '快递',
      supplier: `厦门${ 550 + a}信息有限公司`,
      freightTemplate: `模板名称${a}`,
      accountingRules: `福建   ${a}件内${a + 9}元，每增加${a}件，加${a + 9}元。指定条件包邮：满${a}件包邮`,
      billingMode: '按件数',
      total: `${a + 3}`,
      distributionCost:`${a * 10 }.00`
    })
  }*/
  export default {
    name: 'OrderListList',
...
//**********************************数据渲染开始********************************************//
        ...
        let storeTemplateMaps = data.storeTemplateMaps
        ...
        this.formList = orderStoreSubListDTOs//商品信息数据
        this.storeTemplateMaps = storeTemplateMaps//运费模板数据
        this.card6Cnt = [
          {
            term: '优惠券id',
            ...
            term: '优惠券有效期',
            content: data.marketingDiscountCoupon ? data.marketingDiscountCoupon.startTime + '至' + data.marketingDiscountCoupon.endTime || ' - - ' : ' - - '
          }
