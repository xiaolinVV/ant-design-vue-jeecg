<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      <a-button type="primary" icon="download"  :loading="exportLoading"  @click="handleExportXls('订单-店铺订单-交易成功订单')">导出</a-button>
    </div>
    <!--<div class="table-operator">
...
-->
    ...
    <div>
      ...
      >
        ...
        isSender
        <template slot="isSender" slot-scope="text, record, index">
          <div v-if="record.isSender == 0">否</div>
          <div v-if="record.isSender == 1">是</div>
        </template>
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/orderStoreList/orderStoreList/queryPageListDealsAreDone',
        delete: '/orderStoreList/orderStoreList/delete',
        deleteBatch: '/orderStoreList/orderStoreList/deleteBatch',
        exportXlsUrl: 'orderStoreList/orderStoreList/exportPageListDealsAreDone',
  ...
  methods: {
    ...
    particulars(record, reusePage) {
      ...
      this.$router.push({ path: '/order/OrderSupplierDetails', query: { record: record, reusePage: reusePage } }) //reusePage=5
    },
    //发货
    deliverGoods(record) {
      localStorage.setItem('deliverGoods', JSON.stringify(record))
      this.$router.push({ path: '/order/modules/SupplierOrderDeliverGoods', query: { record: record } }) //reusePage=4
    },
    //查看物流
