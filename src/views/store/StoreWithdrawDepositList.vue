<template>
  <a-card :bordered="false">
    ...
    <div class="table-operator">
      ...
      <!--<a-button type="primary" icon="download"  :loading="exportLoading"  @click="handleExportXls('店铺提现审批')">导出</a-button>-->
    ...
    <div>
      ...
      >
        ...
        <span slot="action" slot-scope="text, record">
          <a v-if="record.status == '0'" @click="showAudiModal(record.id, record.orderNo, record.storeManageId)"
            >审核</a
          >
          <a
            v-if="record.status == '1'"
            @click="showEemitModal(record.id, record.orderNo, record.storeManageId, record)"
            >打款</a
          >
    ...
    <!-- 表单区域 -->
    <storeWithdrawDeposit-modal ref="modalForm" @ok="modalFormOk"></storeWithdrawDeposit-modal>
  </a-card>
</template>

<script>
import StoreWithdrawDepositModal from './modules/StoreWithdrawDepositModal'
import { JeecgListMixin } from '@/mixins/JeecgListMixin'
import { filterObj } from '@/utils/util'
export default {
  name: 'StoreWithdrawDepositList',
  mixins: [JeecgListMixin],
  components: {
    StoreWithdrawDepositModal
  },
  data() {
    return {
      description: '店铺提现审批管理页面',
      // 表头
      columns: [
        {
          title: '#',
      ...
      url: {
        ...
        delete: '/storeWithdrawDeposit/storeWithdrawDeposit/delete',
        deleteBatch: '/storeWithdrawDeposit/storeWithdrawDeposit/deleteBatch',
        exportXlsUrl: 'storeWithdrawDeposit/storeWithdrawDeposit/exportXls',
        importExcelUrl: 'storeWithdrawDeposit/storeWithdrawDeposit/importExcel'
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    showAudiModal(id, orderNo, storeManageId) {
      this.$refs.modalForm.showAudiModal(id, orderNo, storeManageId)
      this.$refs.modalForm.title = '审核'
      this.$refs.modalForm.disableSubmit = false
    },
    /**打款弹窗*/
    showEemitModal(id, orderNo, storeManageId, item) {
      this.$refs.modalForm.showEemitModal(id, orderNo, storeManageId, item)
