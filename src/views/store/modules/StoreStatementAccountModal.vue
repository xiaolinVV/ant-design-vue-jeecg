...
<script>
  ...
  export default {
    name: "StoreStatementAccountModal",
    ...
    data () {
      return {
        ...
        url: {
         // list:"/storeStatementAccount/storeStatementAccount/getStoreAccountCapitalListList",
          add: "/storeStatementAccount/storeStatementAccount/add",
          edit: "/storeStatementAccount/storeStatementAccount/edit",
          getStoreAccountCapitalListList:"/storeStatementAccount/storeStatementAccount/getStoreAccountCapitalListList"
        },
      }
    },
    created () {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'year','month','day','delFlag','earning','incomeNumber','expenditure','expenditureNumber','revenue','startBalance','endBalance','storeManageId'))
		  //时间格式化
     ...
     //商品信息弹窗
      showModalParticularsInformation(id){
        if(!id){
          ...
          this.storeAccountCapitalId = id;
        }
        this.visiblParticularsInformation = true;

        getAction(this.url.getStoreAccountCapitalListList,{id: id,pageNo:this.ipagination.current,pageSize:this.ipagination.pageSize}).then((res) => {
          if (res.success) {
      ...
      loadData(arg) {
        ...
        this.loading = true;
        getAction(this.url.getStoreAccountCapitalListList,{id: this.storeAccountCapitalId,pageNo:this.ipagination.current,pageSize:this.ipagination.pageSize }).then((res) => {
          if (res.success) {
            this.dataSource = res.result.records;
            this.ipagination.total = res.result.total;
          }
