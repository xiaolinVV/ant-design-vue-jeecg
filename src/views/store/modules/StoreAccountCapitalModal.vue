    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          <a-input placeholder="请输入交易类型；0：订单交易；1：余额提现；2：订单退款；3：福利金赠送；4：余额充值；5：归属店铺奖励；6：渠道销售奖励；做成数据字典" v-decorator="['payType', {}]" />
        </a-form-item>
        <a-form-item
          :labelCol="labelCol"
          :wrapperCol="wrapperCol"
  ...
  export default {
    ...
    data () {
      return {
        ...
        url: {
          add: "/storeAccountCapital/storeAccountCapital/add",
          edit: "/storeAccountCapital/storeAccountCapital/edit",
        },
      }
    },
    created () {
    ...
    methods: {
      ...
      edit (record) {
        ...
        this.visible = true;
        this.$nextTick(() => {
          this.form.setFieldsValue(pick(this.model,'year','month','day','delFlag','storeManageId','payType','goAndCome','amount','orderNo'))
		  //时间格式化
        });
