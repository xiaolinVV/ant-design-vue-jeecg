      ...
      <div class="detail">
        <a-form :form="form" >
          <a-form-item label="模板名称" :label-col="{ span: 5 }" :wrapper-col="{ span: 12 }">
            <a-input
              v-decorator="['name', { rules: [{ required: true, message: '请输入模板名称' },{max:50,message:'模板名称字数不超过50个字'}] }]"
      ...
      <div class="detail" >
        ...
        <div class="card-content" v-for="(item,index) in PostageArea">
          ...
          <div class="ipt-cnt" v-show="item.isDelivery == 1">
            <div>
              <a-input-number :min="1":precision="0" v-model="item.count" placeholder="请输入" type="number"/>
...
<script>
  ...
  export default{
    ...
    data(){
      return {
        ...
        AllData:{
          ...
          templateType:'',   //全部包邮  0  不是包邮  1
          delFlag:'0',//删除状态
          isTemplate:'0', //是否默认
        },
        url:{
          adds: "/storeTemplate/storeTemplate/add",
          edits: "/storeTemplate/storeTemplate/edit",
    ...
    created() {
      ...
      let info = JSON.parse(localStorage.getItem('FreightTemplate'))
      if(part){
        //添加返回1  编辑返回2
        this.part = part;
      }
...
//      this.headers = {"X-Access-Token": token}
    ...
    methods: {
      ...
      cityEchoDisplay(info){
        ...
        //第一个id请求其对应的子地址
        if(sz.length<=0 ||  this.areaAddressList.length<=0 || !sz[0] || !this.areaAddressList[0]){
          this.canShowDefaultCity = true
          return ;
        }
          ...
          if(this.part ==  2){
            putAction(url,values).then((res)=>{
              if (res.code == 200){
                ...
                a.$router.push({path:"/store/StoreTemplateList"})
              }else {
