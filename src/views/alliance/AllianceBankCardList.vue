<template>
  <a-card :bordered="false">
    ...
    <!-- 表单区域 -->
<!--    @ok="modalFormOk"-->
    <alliance-bank-card-edit-modal ref="modalForm" ></alliance-bank-card-edit-modal>
  </a-card>
</template>

<script>
  import AllianceBankCardEditModal from './modules/AllianceBankCardEditModal'
  import Vue from 'vue'
  import {getAction, putAction, postAction} from '@/api/manage'
  export default {
    name: "AgencyBankCardList",
    components: {
      AllianceBankCardEditModal
    },
    data () {
      return {
        ...
        bankCardSubmit: true,
        url: {
          findAllianceBankCard: "/allianceBankCard/allianceBankCard/returnBankCard"//返显
        },
      }
    },
    computed: {
...
//      this.headers = {"X-Access-Token": token};
      getAction(this.url.findAllianceBankCard).then(res => {
        if (res) {
