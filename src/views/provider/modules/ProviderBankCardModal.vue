    ...
    >
      <a-form :form="form">
        ...
        <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="开户行所在城市">
          <!-- :default-value="['zhejiang', 'hangzhou', 'xihu']" @change="onChange" -->
          <a-cascader
            :options="bankCitys"
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        add: '/providerBankCard/providerBankCard/add',
        edit: '/providerBankCard/providerBankCard/edit',
        verificationCode: '/storeBankCard/storeBankCard/bankCardByPhone', //获取验证码
        updateOrSaveBankCard: '/providerBankCard/providerBankCard/updateOrSaveBankCard' //设置银行卡or支付宝
      }
    }
  },
  created() {},
    ...
    bankCardEdit() {
      this.form.validateFields((err, values) => {
        if (!err) {
          ...
          // return
          this.$nextTick(() => {
            postAction(this.url.updateOrSaveBankCard, formData).then(res => {
