<template>
  <a-card :bordered="false">
    ...
    <a-modal title="审核" v-model="visibleAudi" @ok="hideModal()" okText="确认" cancelText="取消">
...
export default {
  ...
  data() {
    return {
      ...
      url: {
        list: '/goodList/goodList/GoodAuditListList',
  ...
  methods: {
    ...
    hideModal() {
      ...
      getAction(that.url.updateAuditStatus, {
        id: that.goodId,
        auditExplain: that.auditExplain,
        ...
        if (res.code == 200) {
          ...
          that.$message.warning(res.message || '修改失败')
        }
      })
    },
    ...
    batchAudit() {
      if (!this.url.updateAuditStatusPL) {
        this.$message.error('请设置url.updateAuditStatusPL!')
        return
      }
      if (this.selectedRowKeys.length <= 0) {
        ...
        // }
        this.goodId = this.selectedRowKeys.join(',')
        this.auditStatus = '2'
        this.auditExplain = ''
        this.visibleAudi = true
        /*    this.auditStatus = "2";
        ...
        // getAction(that.url.updateAuditStatusPL, {
        //   id: that.goodId,
        //   auditStatus: that.auditStatus,
        //   auditExplain: that.auditExplain
        // }).then(res => {
