<template>
      ...
      <a-card :bordered="false" style="min-height: 800px">
        <a-row>
          ...
          <a-col style="margin-bottom: 10px" :key="item.id" v-for="item in storeList">
            ...
            <a v-else style="font-weight:bold;color:#000000" @click="goodCuisineListClick(item)">{{item.storeName}}</a>
          </a-col>
        </a-row>
      </a-card>
    </a-col>
  ...
  <a-card :bordered="false" style="min-width: 1400px;min-height: 800px">
   ...
   <!--   <a-button type="primary" icon="download" @click="handleExportXls('菜品列表')">导出</a-button>
    ...
    <a-modal :visible="pictureVisible" :footer="null" @cancel="pictureCancel">
      <img v-if="dataSource[pictureIndex]" alt="example" style="width: 100%" :src="getMainPicture(dataSource[pictureIndex].mainPicture)" />
    </a-modal>
  </a-row>
</template>
