    ...
    <div style="">
      ...
      <a-table
        ...
        <span slot="action" slot-scope="text, record">
         <a @click="showModal(record.id,record.auditStatus)" >审核</a>
        </span>
      </a-table>
    </div>
    <!-- table区域-end -->
  ...
  export default {
    ...
    data () {
      return {
		...
		url: {
      list: "/goodStoreList/goodStoreList/GoodStoreAuditListList",
      ...
      updateAuditStatus: "/goodStoreList/goodStoreList/updateAuditStatus",
      updateAuditStatusPL:"/goodStoreList/goodStoreList/updateAuditStatusPL",
       },
    }
  },
  computed: {
    ...
    methods: {
      ...
      //审核
      showModal(id,auditStatus) {
        this.$refs.modalForm.showModal(id,"2");
      ...
      batchAudit: function () {
        ...
        if (this.selectedRowKeys.length <= 0) {
          ...
          this.goodStoreId = ids
          var that = this
          this.$confirm({
            title: '批量审核',
            content: '您确定要批量审核吗？',
            onOk: function() {
              getAction(that.url.updateAuditStatusPL, { id: that.goodStoreId, auditStatus:  that.auditStatus , auditExplain: that.auditExplain }).then((res) => {
                if (res.success) {
                  that.$message.success(res.message)
                  that.loadData()
