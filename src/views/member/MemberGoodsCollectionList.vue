<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :md="6" :sm="8">
            <a-form-item label="收藏时间">
              <a-range-picker
                style="width: 100%"
                v-model="queryParam.collectionTime"
                format="YYYY-MM-DD"
    ...
    <div class="table-operator">
      <!--<a-button type="primary" icon="download" :loading="exportLoading" @click="handleExportXls('购物车商品')">导出</a-button>-->
    </div>
    <!-- table区域-begin -->
    ...
    <!-- 表单区域 -->
    <memberGoodsCollection-modal ref="modalForm" @ok="modalFormOk"></memberGoodsCollection-modal>
  </a-card>
</template>

<script>
  import MemberGoodsCollectionModal from './modules/MemberGoodsCollectionModal'
  ...
  export default {
    name: "MemberGoodsCollectionList",
    mixins:[JeecgListMixin],
    components: {
      MemberGoodsCollectionModal
    },
    data () {
      return {
        description: '商品收藏管理页面',
        collectionTime:[],
        // 表头
        columns: [
          {
		...
		url: {
          list: "/memberGoodsCollection/memberGoodsCollection/list",
          delete: "/memberGoodsCollection/memberGoodsCollection/delete",
          deleteBatch: "/memberGoodsCollection/memberGoodsCollection/deleteBatch",
          exportXlsUrl: "memberGoodsCollection/memberGoodsCollection/exportXls",
          importExcelUrl: "memberGoodsCollection/memberGoodsCollection/importExcel",
       },
    }
  },
  computed: {
    ...
    methods: {
    getQueryParams(){
    console.log(this.queryParam.collectionTime)
    var param = Object.assign({}, this.queryParam,this.isorter);
    param.field = this.getQueryField();
    param.pageNo = this.ipagination.current;
    param.pageSize = this.ipagination.pageSize;
    delete param.collectionTime; // 时间参数不传递后台
