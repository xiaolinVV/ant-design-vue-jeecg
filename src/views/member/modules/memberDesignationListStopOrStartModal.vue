<template>
  <div>
    ...
    <a-modal title="删除" v-model="visibleDelete" @ok="hideModal()" okText="确认" cancelText="取消">
      ...
      <h3>删除该称号后，该称号失效，对应称号分红资金失效，删除后不可恢复。</h3>
      <p>您确定要删除吗？</p>
      <a-textarea
        placeholder="请输入删除说明"
        v-model="closeExplain"
...
<script>
  ...
  export default {
    name: 'memberDesignationListStopOrStartModal',
    data(){
      return{
        ...
        url: {
          ...
          //删除接口
          delete: "/memberDesignation/memberDesignation/deleteAndExplain",
        }
      }
    },
    methods:{
      ...
      hideModal() {
        ...
        if(this.visibleStopStatus || this.visibleStopTitleFuund){
          ...
          //停用
          putAction(that.url.edit, datas).then((res) => {
            if (res.code == 200) {
      ...
      showStartStatus(record){
        ...
        let that = this
        this.$confirm({
          title: '启用该称号后，该称号生效，对应称号分红资金生效。',
          content: '您确定要启用吗？',
          onOk: function() {
            putAction(that.url.edit, { id: record.id,status: 1 ,closeExplain: "" }).then((res) => {
              if (res.success) {
                that.$message.success(res.message)
                that.$emit('ok')
                // that.loadData()
      ...
      showStartTitleFund(record){
        ...
        this.$confirm({
          ...
          content: '您确定要开启吗？',
          onOk: function() {
            putAction(that.url.edit, { id: record.id,isOpenMoney: 1 ,closeExplain: "" }).then((res) => {
              if (res.success) {
                that.$message.success(res.message)
                that.$emit('ok')
                // that.loadData()
