...
<template>
  <a-card :bordered="false">
    <!--店铺余额-->
    <!--<div >&lt;!&ndash;v-if="storeManage"&ndash;&gt;
      <div class="zijin" style="display: flex;justify-content: space-between;margin-bottom: 20px;font-size: 16px">
        <div>
          <h3>
            <span v-if="storeManage.balance">{{ storeManage.balance }} </span>
            <span v-else> 0 </span>
            <a-icon title="可支取使用的账户余额" type="question-circle" /></h3>
          <div>
            <span style="margin-right: 20px">可用余额(元)</span>

            <a @click="handleEdit2()">充值记录</a>
            <a-divider type="vertical" />
            <a @click="handleEdit1(storeManage.id,)">提现记录</a>
          </div>
        </div>
        <div>
          <h3>
            <span v-if="storeManage.accountFrozen">{{ storeManage.accountFrozen }} </span>
            <span v-else> 0 </span>
            <a-icon title="当前尚未完成的交易钱款总额，结算时钱会从待结算中扣除，进入可用余额。
结算：确认收货7天后结算" type="question-circle" />
          </h3>
          <div>待结算(元)</div>
        </div>
        <div>
          <h3>
            <span v-if="storeManage.unusableFrozen">{{ storeManage.unusableFrozen }} </span>
            <span v-else> 0 </span>
      ...
      <div style="margin-bottom: 40px">
        ...
        <a-button type="primary" @click="Recharge(storeManage.id,storeManage.balance)" >充值</a-button>
        <a-button  @click="withdrawDeposit(storeManage.id,storeManage.balance)"  style="margin-left: 8px">提现</a-button>
          ...
          <template v-if="toggleSearchStatus">
            ...
            <a-col :md="8" :sm="8">
              <a-form-item label="收入支出">
                <a-select v-model="queryParam.goAndCome" placeholder="请选择">
...
<script>
  import StoreRechargeRecordModal from '../store/modules/StoreRechargeRecordModal'
  import {JeecgListMixin} from '@/mixins/JeecgListMixin'
  import {httpAction,getAction} from '@/api/manage'
  import {filterObj} from '@/utils/util';
  export default {
    name: "StoreAccountCapitalList",
    ...
    data () {
      return {
        ...
        url: {
          list: "/storeRechargeRecord/storeRechargeRecord/list",
          queryPageListAndManage: "/storeRechargeRecord/storeRechargeRecord/queryPageListAndManage",
    ...
    methods: {
      ...
      handleEdit2(record){
        //路由跳转
        this.$router.push({ path: '/store/StoreRechargeRecord',})

      },
      handleEdit1(record){
        //路由跳转
        this.$router.push({ path: '/store/StorePayRecord',})
      },
