<template>
  <a-card :bordered="false">
    ...
    <!--<div class="zijin" style="display: flex;justify-content: space-between;margin-bottom: 20px;font-size: 16px">
      <div>
        <h3>
          <span v-if="allianceManage.balance">{{ allianceManage.balance }} </span>
          <span v-else> 0 </span>
          <a-icon title="可支取使用的账户余额" type="question-circle"/>
        </h3>
        <div>
          <span style="margin-right: 20px">可用余额(元)</span>
          <a @click="handleEdit1(allianceManage.id,)">提现记录</a>
        </div>
      </div>
      <div>
        <h3>
          <span v-if="allianceManage.accountFrozen">{{ allianceManage.accountFrozen }} </span>
          <span v-else> 0 </span>
...
结算时钱会从待结算中扣除进入店铺余额" type="question-circle"/>
      ...
      <div>
        <h3>
          <span v-if="allianceManage.unusableFrozen">{{ allianceManage.unusableFrozen }} </span>
          <span v-else> 0 </span>
    ...
    <div style="margin-bottom: 40px">
      <a-button @click="withdrawDeposit(allianceManage.sysUserId,allianceManage.balance)" style="margin-left: 8px">提现
      </a-button>
    ...
    <!--<div class="table-operator">
      ...
      <a-button type="primary" icon="download" :loading="exportLoading" @click="handleExportXls('加盟商余额明细')">导出</a-button>
    ...
    <!-- 表单区域 -->
    <allianceRechargeRecord-modal ref="modalForm" @ok="modalFormOk"></allianceRechargeRecord-modal>
  </a-card>
</template>

<script>
  import AllianceRechargeRecordModal from '../alliance/modules/AllianceSettleAccountsModal'
  import {JeecgListMixin} from '@/mixins/JeecgListMixin'
  import { filterObj } from '@/utils/util';
  import {getAction, postAction} from '@/api/manage'
  export default {
    name: "AllianceRechargeRecordList",
    mixins: [JeecgListMixin],
    components: {
      AllianceRechargeRecordModal
    },
    data () {
      return {
        description: '加盟商余额明细管理页面',
        allianceManage: {},
        // 表头
        columns: [
          {
        ...
        url: {
          list: "/allianceRechargeRecord/allianceRechargeRecord/list",
          delete: "/allianceRechargeRecord/allianceRechargeRecord/delete",
          deleteBatch: "/allianceRechargeRecord/allianceRechargeRecord/deleteBatch",
          exportXlsUrl: "allianceRechargeRecord/allianceRechargeRecord/exportXls",
          importExcelUrl: "allianceRechargeRecord/allianceRechargeRecord/importExcel",
          queryPageListAndManage: "allianceRechargeRecord/allianceRechargeRecord/queryPageListAndManage",
        },
      }
    },
    computed: {
    ...
    methods: {
      ...
      allianceManageOne(){
        getAction(this.url.queryPageListAndManage).then((res) => {
          if (res.success) {
            this.allianceManage = res.result;
          }
        });
      },
      //提现记录页面
      handleEdit1(record){
        //路由跳转
        this.$router.push({path: '/alliance/AllianceSettleAccountsList',})
      },
      /**提现*/
