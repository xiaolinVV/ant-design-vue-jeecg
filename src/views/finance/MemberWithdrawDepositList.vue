<template>
  <a-card :bordered="false">
          ...
          <template v-if="toggleSearchStatus">
            ...
            <a-col :md="8" :sm="8">
              <a-form-item label="身份证号">
                <a-input placeholder="" v-model="queryParam.identityNumber"></a-input>
              </a-form-item>
            </a-col>
            <!--时间选择框-->
            <a-col :md="8" :sm="8">
    ...
    <div class="table-operator">
      ...
      <!--<a-button type="primary" icon="download" :loading="exportLoading" @click="handleExportXls('会员提现审批')">导出</a-button>-->
    ...
    <div>
      ...
      >
        <template slot="withdrawalType" slot-scope="text, record">
        ...
        <span slot="action" slot-scope="text, record">
          ...
          <a v-if="record.status == '0'" @click="showAudiModal(record.id, record.orderNo, record.memberListId)">审核</a>
          <a-divider v-if="record.status == '0'" type="vertical" />
          <a v-if="record.status == '1'" @click="showEemitModal(record.id, record.orderNo, record.memberListId)"
            >打款</a
          >
        </span>
      </a-table>
    ...
    <!-- 表单区域 -->
    <memberWithdrawDeposit-modal ref="modalForm" @ok="modalFormOk"></memberWithdrawDeposit-modal>
  </a-card>
</template>

<script>
import MemberWithdrawDepositModal from '../member/modules/MemberWithdrawDepositModal'
import { JeecgListMixin } from '@/mixins/JeecgListMixin'
import { filterObj } from '@/utils/util'
export default {
  name: 'MemberWithdrawDepositList',
  mixins: [JeecgListMixin],
  components: {
    MemberWithdrawDepositModal
  },
  data() {
    return {
      description: '会员提现审批管理页面',
      // 表头
      columns: [
        {
          title: '#',
      ...
      url: {
        list: '/memberWithdrawDeposit/memberWithdrawDeposit/list',
        delete: '/memberWithdrawDeposit/memberWithdrawDeposit/delete',
        deleteBatch: '/memberWithdrawDeposit/memberWithdrawDeposit/deleteBatch',
        exportXlsUrl: 'memberWithdrawDeposit/memberWithdrawDeposit/exportXls',
        importExcelUrl: 'memberWithdrawDeposit/memberWithdrawDeposit/importExcel'
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    showAudiModal(id, orderNo, memberListId) {
      this.$refs.modalForm.showAudiModal(id, orderNo, memberListId)
      this.$refs.modalForm.title = '审核'
      this.$refs.modalForm.disableSubmit = false
    },
    /**打款弹窗*/
    showEemitModal(id, orderNo, memberListId) {
      this.$refs.modalForm.showEemitModal(id, orderNo, memberListId)
      this.$refs.modalForm.title = '打款'
      this.$refs.modalForm.disableSubmit = false
    },
    /*查看详情弹窗*/
    showDetailModal(record) {
      this.$refs.modalForm.showDetailModal(record)
