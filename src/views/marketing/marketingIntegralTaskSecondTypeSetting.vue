<template>
  <div class="marketingIntegralTaskSecondTypeSetting">
    <div class="title">
      基础设置
    </div>
    <a-form :form="form" class="wrap">
      <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="别名">
      ...
      <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="可用奖励">
        <a-checkbox-group v-decorator="rules.availableRewards">
          <a-checkbox value="0">积分（水滴）</a-checkbox>
          <a-checkbox value="1">专区团参团次数</a-checkbox>
          <a-checkbox value="2">平台优惠券</a-checkbox>
          <a-checkbox value="3">店铺优惠券</a-checkbox>
        </a-checkbox-group>
      </a-form-item>
      ...
      <a-form-item :label-col="labelCol" :wrapper-col="wrapperCol" label="成团时限">
        <a-input-number placeholder="成团时限" v-decorator="rules.timeLimit" :min="1" :precision="2" />
        小时
      </a-form-item> -->
      <!-- 
...
export default {
  name: 'marketingIntegralTaskSecondTypeSetting',
  data() {
    return {
      headers: '',
      ...
      rules: Object.freeze({
        ...
        availableRewards: ['availableRewards', { rules: [{ required: true, message: '请选择可用奖励' }] }]
      ...
      url: Object.freeze({
        ...
        addOrEdit: '/marketingIntegralTaskBaseSetting/marketingIntegralTaskBaseSetting/saveAndUpdate', //新增编辑
        queryByOne: 'marketingIntegralTaskBaseSetting/marketingIntegralTaskBaseSetting/findByOne' //返现
      }),
      //配置
  ...
  methods: {
    getList() {
      getAction(this.url.queryByOne).then(async res => {
        ...
        try {
          ...
          if (allData.availableRewards) {
            allData.availableRewards = allData.availableRewards.split(',')
          } else {
            allData.availableRewards = []
          }
        } catch (e) {}
    ...
    sure() {
      ...
      this.form.validateFields((err, values) => {
        ...
        // }
        if (!err) {
          let AllData = Object.assign({}, this.AllData, values)
          this.$nextTick(() => {
            // let coverPlan = {}
            // let postersCK = {}
            // let surfacePlot = {}
            ...
            console.log(AllData.availableRewards)
            if (
              AllData.availableRewards &&
              AllData.availableRewards.length > 0 &&
              Array.isArray(AllData.availableRewards)
            ) {
              AllData.availableRewards = AllData.availableRewards.join(',')
            } else {
              AllData.availableRewards = ''
            }

            postAction(this.url.addOrEdit, AllData)
              .then(res => {
                if (res.success) {
...
<style lang="less">
.marketingIntegralTaskSecondTypeSetting {
	width: 100%;
	background: white;
