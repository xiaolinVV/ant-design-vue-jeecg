<template>
  <j-modal :title="title" :width="800" :visible="visible" :confirmLoading="confirmLoading" :fullscreen="true"
      ...
      <a-form :form="form">
        <a-card title="基础信息">
          ...
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="图标">
            <kq-upload ref="iconImgUpload" :imgCount="1" explain="尺寸200*200 支持jpg、jpeg、png格式，大小不超过2M"
              @change="e => { iConImages = e }"></kq-upload>
          </a-form-item>
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="分享图">
            <kq-upload ref="shareFigureImagesUpload" :imgCount="1" explain="尺寸800*640 ，支持jpg、jpeg、png格式，大小不超过2M"
              @change="e => { shareFigureImages = e }"></kq-upload>
          </a-form-item>
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="海报图">
            <kq-upload ref="postersFigureImagesUpload" :imgCount="1"
              explain="尺寸1428*2540，支持jpg、jpeg、png格式，大小不超过2M。必须预留好二维码位置。以免影响展示效果。"
              @change="e => { postersFigureImages = e }"></kq-upload>
          </a-form-item>
        ...
        <a-card title="类型信息" style="margin-top: 30px">
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="专区类型">
          ...
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" v-if="model.prefectureType == 1" label="限制">
            <span>每</span><a-input v-model="limitation.time" style="width: 100px;margin-left: 10px" /> <a-select
              v-model="limitation.model" style="width: 100px;margin-left: 10px">
          ...
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" v-if="model.prefectureType == 2" label="赠送">
            <a-select v-model="giveInfo.marketingStorePrefectureListId" style="width: 200px;margin-left: 10px">
              <a-select-option :value="item.id" v-for="(item, index) in marketingStorePrefectureList" :key="index">{{
                item.prefectureName }}</a-select-option>
            </a-select>
        ...
        <a-card title="奖励信息" style="margin-top: 30px">
          ...
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="本店分红">
            <a-input-number style="width: 90%" v-decorator="['storeShare', {}]" />%分红资金
          </a-form-item>
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="称号奖励">
            <select-designation-award v-if="storeInfo && storeInfo.key" ref="selectDesignationAward"
              :storeInfo="storeInfo" @change="selectDesignationAwardChange"></select-designation-award>
          </a-form-item>
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="经销商奖励">
            <a-input-number style="width: 90%" v-decorator="['dealerIncentives', {}]" />%分红资金
          </a-form-item>
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="分享人奖励">
            <a-input-number style="width: 90%" v-decorator="['shareAward', {}]" />%分红资金
          </a-form-item>
          <a-form-item :labelCol="labelCol" :wrapperCol="wrapperCol" label="权益有效期">
            购买后<a-input-number style="width: 90%" v-decorator="['periodOfValidity', {}]" :min="0" :precision="0" />天
          </a-form-item>
        </a-card>
...
export default {
  ...
  methods: {
    ...
    edit (record, param) {
      ...
      this.visible = true;
      this.$nextTick(() => {
        this.form.setFieldsValue(pick(this.model, 'prefectureName', 'rule', 'prefectureType', 'astrict', 'give', 'dividend', 'storeShare', 'titleAward', 'dealerIncentives', 'shareAward', 'status', 'periodOfValidity'))
        //时间格式化
        /*初始化组件数据*/
