<template>
  <a-card :bordered="false">
        ...
        <a-row :gutter="24">
          ...
          <a-col :md="8" :sm="8">
            <a-form-item label="跳转类型">
              <a-select v-model="queryParam.goToType" placeholder="请选择跳转类型">
    ...
    <div>
      ...
      >
        <template slot="pictureAddr" slot-scope="text, record, index">
          <img
        ...
        <template slot="adAdress" slot-scope="text, record, index">
          <img class="clickShowImage" v-if="text" :preview="'adAdress' + index" :src="handleImage(text)" alt="" />
          <div v-else>
            无
          </div>
        </template>

        <template slot="goToType" slot-scope="text">
          {{ text == 1 ? '图片详情' : '无' }}
        </template>
    ...
    <!-- 表单区域 -->
    <marketingGroupIntegralAdvertising-modal
      ref="modalForm"
      @ok="modalFormOk"
    ></marketingGroupIntegralAdvertising-modal>
    <textAreaModal
...
<script>
import MarketingGroupIntegralAdvertisingModal from './modules/MarketingGroupIntegralAdvertisingModal'
import { JeecgListMixin } from '@/mixins/JeecgListMixin'
import { filterObj } from '@/utils/util'
import textAreaModal from '@/components/popUp/textAreaModal'
import { putAction, deleteAction } from '@/api/manage'
export default {
  name: 'MarketingGroupIntegralAdvertisingList',
  mixins: [JeecgListMixin],
  components: {
    MarketingGroupIntegralAdvertisingModal,
    textAreaModal
  },
  data() {
    return {
      description: '拼购广告管理页面',
      // 表头
      columns: [
        {
          title: '#',
        ...
        },
        {
          ...
          align: 'center',
          dataIndex: 'adAdress',
          scopedSlots: { customRender: 'adAdress' }
        },
      ...
      modalForm2Infos: {},
      url: {
        list: '/marketing/marketingGroupIntegralAdvertising/list',
        delete: '/marketing/marketingGroupIntegralAdvertising/delete',
        deleteBatch: '/marketing/marketingGroupIntegralAdvertising/deleteBatch',
        exportXlsUrl: 'marketing/marketingGroupIntegralAdvertising/exportXls',
        importExcelUrl: 'marketing/marketingGroupIntegralAdvertising/importExcel',
