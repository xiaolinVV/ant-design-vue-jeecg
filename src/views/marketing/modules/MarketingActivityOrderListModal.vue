    ...
    >
      <div v-if="memberList">
        ...
        <div class="CouponTextA">
          ...
          <div class="CouponText2">{{ memberList.memberGrad }}</div>
        </div>
        <div class="CouponTextA">
    ...
    >
      <div v-for="item in formList" style="margin-bottom: 20px">
        <div class="suppliers">
          <div>
            <span>供应商</span>
            <span class="suppliers-content">{{ item.providerManageName }}</span>
          </div>
          <div>
            <span>供应商订单编号</span>
...
export default {
  name: 'MarketingActivityOrderListModal',
  data() {
    return {
      record: {},
      title: '操作',
      ...
      url: {
        ...
        //根据订单id查询供应商信息和商品信息（1）
        getOrderProviderListAndGoodListByOrderId:
          'orderProviderList/orderProviderList/getOrderProviderListAndGoodListByOrderId',
        // 根据订单id查询订单运费模板信息(1)
        getOrderProviderTemplateByOrderId:
          'orderProviderTemplate/orderProviderTemplate/getOrderProviderTemplateByOrderId'
      }
    }
  },
  computed: {
  ...
  methods: {
    ...
    showModalGoodsInformation(record) {
      console.log(record)
      getAction(this.url.getOrderProviderListAndGoodListByOrderId, { orderListId: record.orderListId }).then(res => {
        if (res.success) {
          const orderProviderListDTOs = res.result
    ...
    showModalDistributionInformation(record) {
      getAction(this.url.getOrderProviderTemplateByOrderId, { orderListId: record.orderListId }).then(res => {
        if (res.success) {
          const providerTemplateMaps = res.result
