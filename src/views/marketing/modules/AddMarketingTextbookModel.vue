    ...
    <a-skeleton :loading="skeletonLoading">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          label="初始浏览量"
        >
          <a-input-number :min="0" v-decorator="[ 'initialViews', validatorRules.initialViews]" style="width: 250px"></a-input-number>
        </a-form-item>
        ...
        <div style="display: flex;align-items: center;justify-content: space-between;width: 150px;height: 100px;margin:0 auto">
          <a-button type="white">
            取消
          </a-button>
  ...
  export default {
    ...
    data() {
      return {
        ...
        validatorRules: {
          marketingTextbookColumnId: { rules: [{ required: true, message: '请选择栏目!' }] },
          ...
          sort :{ rules: [{ required: false, message: '请输入排序!' }] },
        },
        //封面图配置
        frontCoverFileList: [],
        frontCoverPic: true,
    ...
    async created() {
      ...
      let AddMarketingTextbookModelData = localStorage.getItem('AddMarketingTextbookModelData')
      if(AddMarketingTextbookModelData){
        AddMarketingTextbookModelData = JSON.parse(AddMarketingTextbookModelData)
        if(!AddMarketingTextbookModelData.id){
          AddMarketingTextbookModelData = ''
        }
      }else{
        AddMarketingTextbookModelData = ''
      }
      const getMarketingMaterialColumnList = await this.getMarketingMaterialColumnList();
      if(isEdit == 2 || AddMarketingTextbookModelData){
        let AddMarketingMaterialModelDataResult = AddMarketingTextbookModelData
        ...
        this.twContent = AddMarketingTextbookModelData.content
        // this.dataSource = AddMarketingTextbookModelData.marketingMaterialGoodDTOList
        this.AllData = Object.assign({},this.AllData, AddMarketingTextbookModelData);
        this.$nextTick(()=>{
        ...
        this.$nextTick(()=>{
          this.form.setFieldsValue(this.AllData)
          localStorage.removeItem('AddMarketingTextbookModelData')
        })
      }
    ...
    methods: {
      ...
      videoBeforeUpload(file) {
        const isVideo = file.type === 'video/mp4' || file.type === 'video/rmvb' || file.type === 'video/3gp' || file.type === 'video/avi' || file.type === 'video/mov' || file.type === 'video/x-ms-wmv' || file.type === 'video/flv' || file.type === 'video/mkv'
        if (!isVideo) {
            ...
            this.allHandleImg(()=>{
              ...
              httpAction(httpurl,values,method).then((res)=>{
                if(res.success){
                  ...
                  this.$router.push({ path: '/marketing/MarketingTextbookListList'})
                }else{
