<template>
  <a-modal
    :title="title"
    :width="800"
    :visible="visible"
    :confirmLoading="confirmLoading"
    @ok="handleOk"
    @cancel="handleCancel"
    cancelText="关闭">
    <div v-if="title == '启用'">
      <div class="big-title">
        <i aria-label="icon: question-circle" class="anticon anticon-question-circle">
          <svg viewBox="64 64 896 896" data-icon="question-circle" width="1em" height="1em" fill="currentColor"
               aria-hidden="true" focusable="false" class="">
            <path
              d="M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64zm0 820c-205.4 0-372-166.6-372-372s166.6-372 372-372 372 166.6 372 372-166.6 372-372 372z"></path>
            <path
              d="M623.6 316.7C593.6 290.4 554 276 512 276s-81.6 14.5-111.6 40.7C369.2 344 352 380.7 352 420v7.6c0 4.4 3.6 8 8 8h48c4.4 0 8-3.6 8-8V420c0-44.1 43.1-80 96-80s96 35.9 96 80c0 31.1-22 59.6-56.1 72.7-21.2 8.1-39.2 22.3-52.1 40.9-13.1 19-19.9 41.8-19.9 64.9V620c0 4.4 3.6 8 8 8h48c4.4 0 8-3.6 8-8v-22.7a48.3 48.3 0 0 1 30.9-44.8c59-22.7 97.1-74.7 97.1-132.5.1-39.3-17.1-76-48.3-103.3zM472 732a40 40 0 1 0 80 0 40 40 0 1 0-80 0z"></path>
          </svg>
        </i>
        启用后，该兑换券将在该推荐位展示。
      </div>
      <div class="small-title">
        您确定要启用吗？
      </div>
    </div>
    <div v-if="title == '停用'">
      <div class="big-title">
        <i aria-label="icon: question-circle" class="anticon anticon-question-circle">
          <svg viewBox="64 64 896 896" data-icon="question-circle" width="1em" height="1em" fill="currentColor"
               aria-hidden="true" focusable="false" class="">
            <path
              d="M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64zm0 820c-205.4 0-372-166.6-372-372s166.6-372 372-372 372 166.6 372 372-166.6 372-372 372z"></path>
            <path
              d="M623.6 316.7C593.6 290.4 554 276 512 276s-81.6 14.5-111.6 40.7C369.2 344 352 380.7 352 420v7.6c0 4.4 3.6 8 8 8h48c4.4 0 8-3.6 8-8V420c0-44.1 43.1-80 96-80s96 35.9 96 80c0 31.1-22 59.6-56.1 72.7-21.2 8.1-39.2 22.3-52.1 40.9-13.1 19-19.9 41.8-19.9 64.9V620c0 4.4 3.6 8 8 8h48c4.4 0 8-3.6 8-8v-22.7a48.3 48.3 0 0 1 30.9-44.8c59-22.7 97.1-74.7 97.1-132.5.1-39.3-17.1-76-48.3-103.3zM472 732a40 40 0 1 0 80 0 40 40 0 1 0-80 0z"></path>
          </svg>
        </i>
        停用后,该兑换券将不在该推荐位展示。
      </div>
      <div class="small-title">
        您确定要停用吗？
      </div>
      <a-textarea placeholder="请输入停用说明" :rows="4" v-model="closeExplain"/>
    </div>
    <div v-if="title == '删除'">
      <div class="big-title">
        <i aria-label="icon: question-circle" class="anticon anticon-question-circle">
          <svg viewBox="64 64 896 896" data-icon="question-circle" width="1em" height="1em" fill="currentColor"
               aria-hidden="true" focusable="false" class="">
            <path
              d="M512 64C264.6 64 64 264.6 64 512s200.6 448 448 448 448-200.6 448-448S759.4 64 512 64zm0 820c-205.4 0-372-166.6-372-372s166.6-372 372-372 372 166.6 372 372-166.6 372-372 372z"></path>
            <path
              d="M623.6 316.7C593.6 290.4 554 276 512 276s-81.6 14.5-111.6 40.7C369.2 344 352 380.7 352 420v7.6c0 4.4 3.6 8 8 8h48c4.4 0 8-3.6 8-8V420c0-44.1 43.1-80 96-80s96 35.9 96 80c0 31.1-22 59.6-56.1 72.7-21.2 8.1-39.2 22.3-52.1 40.9-13.1 19-19.9 41.8-19.9 64.9V620c0 4.4 3.6 8 8 8h48c4.4 0 8-3.6 8-8v-22.7a48.3 48.3 0 0 1 30.9-44.8c59-22.7 97.1-74.7 97.1-132.5.1-39.3-17.1-76-48.3-103.3zM472 732a40 40 0 1 0 80 0 40 40 0 1 0-80 0z"></path>
          </svg>
        </i>
        删除后该券将不再该推荐位展示。
      </div>
      <div class="small-title">
        您确定要删除吗？
      </div>
    </div>

  </a-modal>
</template>

<script>
  import { deleteAction, getAction, downFile, postAction, putAction } from '@/api/manage'

  export default {
    inject: ['rush'],
    name: 'MarketingRecommendCertificateModalActionModel',
    data() {
      return {
        title: '',
        visible: false,
        confirmLoading: false,
        allData: {},
        closeExplain: '',
        url: {
          delete: '/marketingRecommendCertificate/marketingRecommendCertificate/delete',
          edit: '/marketingRecommendCertificate/marketingRecommendCertificate/edit'
        }
      }
    },
    methods: {
      handleOk() {
        this.confirmLoading = true
        if (this.title == '删除') {
          this.handleDelete()
        } else {
          this.statusChange()
        }
      },
      handleCancel() {
        this.visible = false
      },
      handleDelete() {
        if (!this.url.delete) {
          this.$message.error('请设置url.delete属性!')
          return
        }
        let that = this
        deleteAction(that.url.delete, { id: this.allData.id }).then((res) => {
          this.confirmLoading = false
          this.handleCancel()
          if (res.success) {
            that.$message.success(res.message)
            that.rush()
          } else {
            that.$message.warning(res.message)
          }
        })
      },
      statusChange() {
        let requestInfo = {
          id: this.allData.id,
          closeExplain: this.closeExplain,
          status: 1
        }
        let message
        let that = this
        if (this.title == '启用') {
          requestInfo.status = 1
          message = '启用成功！'
        } else if (this.title == '停用') {
          requestInfo.status = 0
          message = '停用成功！'
        }
        putAction(this.url.edit, requestInfo).then(res => {
          this.confirmLoading = false
          this.handleCancel()
          if (res.success) {
            that.$message.success(message)
            that.rush()
          } else {
            that.$message.warning(res.message)
          }
        })
      },
      open(result) {
        console.log(result)
        this.visible = true
        this.title = result.modelTitle
        this.allData = result
      }
    }
  }
</script>

<style scoped lang="less">
  i {
    color: #faad14;
    font-size: 22px;
    margin-right: 15px;
  }

  .big-title {
    font-size: 16px;
    color: black;
    margin-bottom: 10px;
    display: flex;
    align-items: center;
  }

  .small-title {
    font-size: 12px;
    color: #999999;
    margin-bottom: 10px;
  }
</style>