  ...
  data() {
    return {
      ...
      url: {
        ...
        //提交接口
        chooseAddGood: 'marketingZoneGroupGood/marketingZoneGroupGood/addArr',
        //获取中奖拼团商品类型（1）
  ...
  async created() {
    ...
    let marketingZoneGroupRecord =
      (this.$route.query.marketingZoneGroupRecord && this.$route.query.marketingZoneGroupRecord.length > 0
        ...
        : {}) || {}
    this.marketingZoneGroupId = this.$route.query.marketingZoneGroupId || ''
    this.marketingZoneGroupRecord = marketingZoneGroupRecord
    this.limitLsit[0].value = marketingZoneGroupRecord.id
    this.limitLsit[1].value = marketingZoneGroupRecord.zoneName
    ...
    async handleSubmit(e) {
      ...
      postAction(this.url.chooseAddGood, JSON.stringify(resALL), {
        ...
        if (res.success) {
          ...
          setTimeout(() => {
            this.$router.push({ path: '/marketing/MarketingZoneGroupGoodList' })
          }, 500)
        } else {
