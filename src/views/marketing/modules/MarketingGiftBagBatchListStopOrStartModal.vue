<template>
  <div>
    <a-modal title="停用" v-model="visibleStopStatus" @ok="hideModal()" okText="确认" cancelText="取消">
      ...
      <h3>停用礼包后，买家之前领取的礼包，在可用时间内还能继续使用，买家无法购买</h3>
      <p>您确定要停用吗？</p>
      <a-textarea
        placeholder="请输入停用说明"
    ...
    <a-modal title="删除" v-model="visibleDelete" @ok="hideModal()" okText="确认" cancelText="取消">
      ...
      <h3>删除礼包后，已领取的礼包包内所有奖励也将会被作废</h3>
      <p>您确定要删除吗？</p>
      <a-textarea
        placeholder="请输入删除说明"
        v-model="closeExplain"
    ...
    data(){
      return{
        ...
        url: {
          ...
          edit: "marketingGiftBagBatch/marketingGiftBagBatch/updateMarketingGiftBagBatch",
          //删除接口
    ...
    methods:{
      ...
      hideModal() {
        ...
        if(this.visibleDelete){
          deleteAction(that.url.delete, {id: that.model.id,delExplain:this.closeExplain}).then((res) => {
            if (res.success) {
              that.$message.success(res.message);
              that.$emit('ok')//发送父级主键刷新
            } else {
