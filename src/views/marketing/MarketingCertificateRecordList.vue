...
<template>
  <a-card :bordered="false">
      ...
      <a-form layout="inline">
        <a-row :gutter="24">
          ...
          <template v-if="toggleSearchStatus">
            ...
            <a-col :md="8" :sm="8">
              <a-form-item label="获赠人">
                <a-input placeholder="请输入获赠人" v-model="queryParam.giveName"></a-input>
              </a-form-item>
            </a-col>
          </template>
          <a-col :md="6" :sm="8">
            <span style="float: left;overflow: hidden;" class="table-page-search-submitButtons">
              ...
              <a-button type="primary" @click="searchReset" icon="reload" style="margin-left: 8px">重置</a-button>
               <a-button type="primary" icon="plus" style="margin-left: 8px" @click="sendCouponModel">送兑换券</a-button>
    ...
    <div>
      ...
      <a-table
        ...
        <template slot="stores" slot-scope="text, record, index">
          <div class="anty-img-wrap">
            <a @click="showStoreInfo(record.marketingCertificateId)" v-if="record.stores>0">{{record.stores}}</a>
            <span v-else>全平台</span>
          </div>
        </template>

        <template slot="delName" slot-scope="text, record, index">
          <div class="anty-img-wrap">
            {{record.delName?record.delName:'~'}}
          </div>
        </template>
    ...
    <marketing-discount-couponList-tarrace-send-couponModel ref="MarketingDiscountCouponListTarraceSendCouponModel"
                                                            @success="loadData">

    </marketing-discount-couponList-tarrace-send-couponModel>
  </a-card>
</template>

<script>
  import MarketingCertificateRecordModal from './modules/MarketingCertificateRecordModal'
  import MarketingDiscountCouponListTarraceSendCouponModel
    from './modules/MarketingDiscountCouponListTarraceSendCouponModel'
  ...
  export default {
      ...
      return {
        ...
        columns: [
          ...
          },
          {
            ...
            align: 'center',
            dataIndex: 'delName',
            scopedSlots: { customRender: 'delName' }
          }
          /*{
            title: '操作',
            dataIndex: 'action',
        ...
        url: {
          list: '/marketingCertificateRecord/marketingCertificateRecord/findCertificateRecord',
    ...
    methods: {
      ...
      sendCouponModel(value = {}) {
        value.modalTitle = '送兑换券'
        this.$refs.MarketingDiscountCouponListTarraceSendCouponModel.open(value)
      },
      //适用商品弹窗
      ...
      getQueryParams() {
        ...
        delete param.useTime // 时间参数不传递后台
      ...
      onDateChange2: function(value, dateString) {
        this.queryParam.useTime_begin = dateString[0]
        this.queryParam.useTime_end = dateString[1]
      },
      onDateOk(value) {
        console.log(value)
      }
